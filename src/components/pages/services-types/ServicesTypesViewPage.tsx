...
import { getAllServiceTypes } from "@/services/loadData";
...
const initSearchValues = new SearchDto({
  ...
  selectColumns: ["id", "name", "systemOfServiceName", "womenIsRequired", "isActive", "names"],
});

export default function ServicesTypesViewPage() {
  const { t, i18n } = useTranslation();
  const { isHttpClientLoading } = useAppStore();

  const [rows, setRows] = useState<IServiceType[]>([]);
  ...
  const [deletedBranch, setDeletedBranch] = useState<IServiceType | null>(null);
  ...
  function fetchBranches(params: SearchDto) {
    getAllServiceTypes(i18n.language, params)
      .then((res) => {
        const indexedRows =
  ...
  async function handleDelete(id?: string) {
    if (!id) return;
    try {
      const response = await fetchClient<ResultHandler<null>>(EndPointsEnums.serviceType, {
        method: "DELETE",
        params: {
          id,
        },
  ...
  async function handleUpdateStatus(id?: string) {
    ...
    try {
      const response = await fetchClient<ResultHandler<null>>(EndPointsEnums.serviceType + "/UpdateStatus", {
        method: "PUT",
        params: {
          id: id,
        },
  ...
  const columns: GridColDef<IServiceType>[] = [
    { field: "index", headerName: t("Service code"), minWidth: 150, headerClassName: "bg-[var(--tableHead)]", headerAlign: "left" },
    {
      field: "name",
    ...
    },
    {
      ...
      headerName: t("Requires a woman in the house"),
      headerClassName: "bg-[var(--tableHead)]",
      sortable: true,
      headerAlign: "center",
      align: "center",
    ...
    },
    {
      ...
      renderCell: (params) => (
        <div className="flex h-full items-center gap-2">
          ...
          <Link
            href={"/services-types/" + params.row.id + "/edit"}
    ...
    <div className="flex-grow w-full flex flex-col p-5">
      <div className="title_actions_btn">
        <div className="flex justify-between items-center">
          <h1 className="text-[18px] font-normal text-[#195950] mb-0">{t("View the services type in the system")}</h1>
          <Link className="main_button" href={"/services-types/add"}>
      ...
      >
        <Box sx={modalStyle}>
          ...
          <div className="text-center">
            <h3 className="font-bold text-[20px]">{t("Delete service type?")}</h3>
            <p className="font-normal text-[18px] mt-2">
              {t(
                "When you delete the service type, you will lose all the service type information and it will be transferred to the deleted list",
              )}
            </p>
          </div>
