...
import { getAllSystemOfService } from "@/services/loadData";
...
export default function ServiceSystemsViewPage() {
  const { t, i18n } = useTranslation();
  const { isHttpClientLoading } = useAppStore();

  const [rows, setRows] = useState<IServicesSystems[]>([]);
  ...
  const [deletedJob, setDeletedJob] = useState<IServicesSystems | null>(null);
  ...
  function fetchBranches(params: SearchDto) {
    getAllSystemOfService(i18n.language, params)
      .then((res) => {
        const indexedRows =
  ...
  async function handleDelete(id?: string) {
    if (!id) return;
    try {
      const response = await fetchClient<ResultHandler<null>>(EndPointsEnums.systemOfService, {
        method: "DELETE",
        params: {
          id,
        },
  ...
  async function handleUpdateStatus(id?: string) {
    ...
    try {
      const response = await fetchClient<ResultHandler<null>>(EndPointsEnums.systemOfService + "/UpdateStatus", {
        method: "PUT",
        params: {
          id: id,
        },
  ...
  const columns: GridColDef<IServicesSystems>[] = [
    {
      field: "index",
      headerName: t("Service code"),
      minWidth: 200,
      sortable: true,
...

    {
      ...
      renderCell: (params) => (
        <div className="flex h-full items-center gap-2">
          ...
          <Link
            href={"/systems-services/" + params.row.id + "/edit"}
    ...
    <div className="flex-grow w-full flex flex-col p-5">
      <div className="title_actions_btn">
        <div className="flex justify-between items-center">
          <h1 className="text-[18px] font-normal text-[#195950] mb-0">{t("View the systems services in the system")}</h1>
          <Link className="main_button" href={"/systems-services/add"}>
            <HiPlusCircle className="w-6 h-6" />
            {t("Add system service")}
          </Link>
        </div>
      </div>
      ...
      <Modal open={!!deletedJob} onClose={handleClose} aria-labelledby="modal-modal-title" aria-describedby="modal-modal-description">
        <Box sx={modalStyle}>
          ...
          <div className="text-center">
            <h3 className="font-bold text-[20px]">{t("Delete system service?")}</h3>
            <p className="font-normal text-[18px] mt-2">
              {t(
                "When you delete the system service, you will lose all the system service information and it will be transferred to the deleted list",
              )}
            </p>
          </div>
