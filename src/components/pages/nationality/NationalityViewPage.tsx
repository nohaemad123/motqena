...
import { getAllNationalities } from "@/services/loadData";
...
export default function NationalityViewPage() {
  const { t, i18n } = useTranslation();
  const { isHttpClientLoading } = useAppStore();

  const [rows, setRows] = useState<INationality[]>([]);
  ...
  const [deletedJob, setDeletedJob] = useState<INationality | null>(null);
  ...
  function fetchBranches(params: SearchDto) {
    getAllNationalities(i18n.language, params)
      .then((res) => {
        const indexedRows =
  ...
  async function handleUpdateStatus(id?: string) {
    ...
    try {
      const response = await fetchClient<ResultHandler<null>>(EndPointsEnums.nationality + "/UpdateStatus", {
        method: "PUT",
        params: {
          id: id,
        },
  ...
  async function handleDelete(id?: string) {
    if (!id) return;
    try {
      const response = await fetchClient<ResultHandler<null>>(EndPointsEnums.nationality, {
        method: "DELETE",
        params: {
          id,
        },
  ...
  const columns: GridColDef<INationality>[] = [
    {
      field: "index",
      headerName: t("Nationality code"),
      minWidth: 200,
      sortable: true,
...

    {
      ...
      renderCell: (params) => (
        <div className="flex h-full items-center gap-2">
          ...
          <Link
            href={"/nationality/" + params.row.id + "/edit"}
    ...
    <div className="flex-grow w-full flex flex-col p-5">
      <div className="title_actions_btn">
        <div className="flex justify-between items-center">
          <h1 className="text-[18px] font-normal text-[#195950] mb-0">{t("View the nationalities in the system")}</h1>
          <Link className="main_button" href={"/nationality/add"}>
      ...
      <Modal open={!!deletedJob} onClose={handleClose} aria-labelledby="modal-modal-title" aria-describedby="modal-modal-description">
        <Box sx={modalStyle}>
          ...
          <div className="text-center">
            <h3 className="font-bold text-[20px]">{t("Delete nationality?")}</h3>
            <p className="font-normal text-[18px] mt-2">
              {t(
                "When you delete the nationality, you will lose all the nationality information and it will be transferred to the deleted list",
              )}
            </p>
          </div>
