...
import { getAllShifts } from "@/services/loadData";
...
const initSearchValues = new SearchDto({
  selectColumns: ["id", "name", "isActive", "time"],
});
...
export default function ShiftViewPage() {
  ...
  const [deletedBranch, setDeletedBranch] = useState<IShift | null>(null);
  ...
  function fetchBranches(params: SearchDto) {
    getAllShifts(i18n.language, params)
      .then((res) => {
        const indexedRows =
  ...
  async function handleDelete(id?: string) {
    if (!id) return;
    try {
      const response = await fetchClient<ResultHandler<null>>(EndPointsEnums.shift, {
        method: "DELETE",
        params: {
          id,
        },
  ...
  async function handleUpdateStatus(id?: string) {
    ...
    try {
      const response = await fetchClient<ResultHandler<null>>(EndPointsEnums.shift + "/UpdateStatus", {
        method: "PUT",
        params: {
          id: id,
        },
  ...
  const columns: GridColDef<IShift>[] = [
    { field: "index", headerName: t("Shift code"), minWidth: 150, headerClassName: "bg-[var(--tableHead)]", headerAlign: "left" },
    // { field: "name", headerName: t("Branch code"), minWidth: 150, sortable: true },
    {
      field: "name",
      headerName: t("Shift name"),
      minWidth: 150,
      flex: 1,
      sortable: true,
      headerAlign: "center",
    ...
    {
      field: "time",
      headerName: t("Duration by hours"),
      minWidth: 350,
      sortable: true,
      headerAlign: "center",
      align: "center",
    ...
    },
    {
      ...
      renderCell: (params) => (
        <div className="flex h-full items-center gap-2">
          ...
          <Link
            href={"/shifts/" + params.row.id + "/edit"}
    ...
    <div className="flex-grow w-full flex flex-col p-5">
      <div className="title_actions_btn">
        <div className="flex justify-between items-center">
          <h1 className="text-[18px] font-normal text-[#195950] mb-0">{t("View the shifts in the system")}</h1>
          <Link className="main_button" href={"/shifts/add"}>
      ...
      >
        <Box sx={modalStyle}>
          ...
          <div className="text-center">
            <h3 className="font-bold text-[20px]">{t("Delete shift?")}</h3>
            <p className="font-normal text-[18px] mt-2">
              {t("When you delete the shift, you will lose all the shift information and it will be transferred to the deleted list")}
            </p>
          </div>
